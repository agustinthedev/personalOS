    ...
    <div>
      <ReactMarkdown
        components={{
          ...
          h3: ({ children }) => (
            <h3 className="mb-4 mt-9 text-xl font-semibold leading-tight text-zinc-50">
              {children}
            </h3>
          ),
          p: ({ children }) => (
          ...
          blockquote: ({ children }) => (
            <blockquote className="my-6 rounded-[28px] border border-white/10 bg-white/[0.02] p-5 text-zinc-300">
              {children}
            </blockquote>
          ),
        }}
